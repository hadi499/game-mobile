...
const THEMES: Record<string, { name: string; images: any[]; color: string; colorLight: string; colorDark: string; bg: string; border: string; shadow: string; text: string }> = {
  fruits: {
    name: 'Buah (Gambar)',
    images: [
      require('../assets/images/memory/apple.png'),
      require('../assets/images/memory/banana.png'),
      require('../assets/images/memory/orange.png'),
      require('../assets/images/memory/grape.png'),
    ],
    color: '#EF4444',
    colorLight: '#FEE2E2',
    colorDark: '#DC2626',
    ...
    >
      <Animated.View style={[{ width: cardSize, height: cardSize }, containerStyle]}>
        ...
        >
          <Image 
            source={card.image} 
            style={{ width: cardSize * 0.6, height: cardSize * 0.6, resizeMode: 'contain' }} 
          />
        </Animated.View>
...
export default function MemoryGameImageScreen() {
  ...
  const cardCount = Math.min(parseInt(params.count || '4', 10), 4);

  const theme = THEMES[themeKey] || THEMES.fruits;
  const images = theme.images.slice(0, cardCount);
  const CARD_GAP = 10;
  ...
  const startGame = useCallback(() => {
    ...
    const newCards = shuffled.map((img, index) => ({
      id: index + 1,
      number: index + 1,
      image: img,
    }));
          ...
          <View style={styles.gameContent}>
            ...
            {(gameState === 'guessing' || gameState === 'result') && (
              <Animated.View entering={FadeIn} style={styles.guessContent}>
                ...
                {targetCard && (
                  <View style={[styles.targetImageContainer, { borderColor: theme.color }]}>
                    <Image 
